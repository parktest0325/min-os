...
extern "C" void main(int argc, char** argv) {
  auto [layer_id, err_openwin] = SyscallOpenWindow(kCanvasSize + 8, kCanvasSize + 28, 10, 10, "eye");
  if (err_openwin) {
    exit(err_openwin);
  }
  ...
  while (true) {
    ...
    if (events[0].type == AppEvent::kQuit) {
      ...
      SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, 4, 24, kCanvasSize, kCanvasSize, 0xffffff);
