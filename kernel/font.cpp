...
WithError<FT_Face> NewFTFace() {
  ...
  if (int err = FT_New_Memory_Face(
        ft_library, korean_buf->data(), korean_buf->size(), 0, &face)) {
...
void InitializeFont() {
  ...
  auto [ entry, pos_slash ] = fat::FindFile("/resource/korddm.ttf");
  if (entry == nullptr || pos_slash) {
    exit(1);
  }

  const size_t size = entry->file_size;
  korean_buf = new std::vector<uint8_t>(size);
  if (LoadFile(korean_buf->data(), size, *entry) != size) {
    delete korean_buf;
    exit(1);
  }
}