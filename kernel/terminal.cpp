...
WithError<int> MakeArgVector(char* command, char* first_arg, 
  ...
  // first_arg: "argv[1] argv[2] argv[3] ... \x00"
  char* p = first_arg;
  while (true) {
    if (p[0] == 0) {
      break;
...
WithError<uint64_t> CopyLoadSegments(Elf64_Ehdr* ehdr) {
  ...
  for (int i = 0; i < ehdr->e_phnum; ++i) {
    ...
    const auto num_4kpages = ((phdr[i].p_vaddr & 4095)
...
void Terminal::ExecuteLine() {
  ...
  if (strcmp(command, "echo") == 0) {
    ...
    const size_t total_cnt = task_manager->TotalTaskCount();
    const size_t running_cnt = task_manager->RunningTaskCount();
    const auto p_stat = memory_manager->Stat();

    PrintToFD(*files_[1], "total task: %lu, running task: %lu\n", total_cnt, running_cnt);
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry& file_entry, char* command, char* first_arg) {
  ...
  const int stack_size = 0x100 * 4096;
...
void Terminal::Print(const char* s, std::optional<size_t> len) {
  ...
  // 글을 쓴 부분영역이 맨 위가 아닐 수 있어서 해당되는 영역은 다시 그려져야 한다.
